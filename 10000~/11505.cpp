...
ll update(ll idx, ll val, ll node, ll left, ll right) {
	...
	ll mid = (left + right) / 2;
	return sTree[node] = update(idx, val, node * 2, left, mid) * update(idx, val, node * 2 + 1, mid + 1, right) % 1000000007;
}
...
ll query(ll start, ll end, ll node, ll left, ll right) {
	...
	ll mid = (left + right) / 2;
	return query(start, end, node * 2, left, mid) * query(start, end, node * 2 + 1, mid + 1, right) % 1000000007;
}
